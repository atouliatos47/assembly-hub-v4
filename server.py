...
def save_state():
    try:
        data = {
            'centers': {k: {key: v for key, v in c.items() if key != 'connected'} for k, c in centers.items()},
            'documents': documents
        }
...
def load_state():
    ...
    try:
        with open(STATE_FILE, 'r') as f:
            data = json.load(f)
        for k, c in data.get('centers', {}).items():
            c['connected'] = False
            centers[k] = c
        for k, d in data.get('documents', {}).items():
            # Only restore if the original file still exists
            if os.path.exists(d.get('filePath', '')):
                documents[k] = d
            else:
                print(f'⚠ Skipping missing file: {d.get("name")}')
        print(f'✅ State loaded: {len(centers)} centers, {len(documents)} documents')
    except Exception as e:
